# CORE Document Disposition

Documents: 60
Review: 19
Recommended destinations: 46
Regional destinations: 41
Canonical families: 7

No files are moved automatically.
## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/DESERT/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'DESERT', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/DESERT/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/COAST/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/COAST/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_PARTNERSHIP.md
- Subject: `partnership`
- Canonical family: `FAM-5af9166c8ae0` / `partnership`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/GOVERNANCE_AND_AUTHORITY.md
- Subject: `governance_authority`
- Canonical family: `FAM-9fb17d14be42` / `governance_authority`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/RIVER/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'RIVER', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/RIVER/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/PLAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/RIVER/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'RIVER', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/RIVER/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/PLAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_PARTNERSHIP.md
- Subject: `partnership`
- Canonical family: `FAM-5af9166c8ae0` / `partnership`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/CULTURAL_AUDIT_CHECKLIST.md
- Subject: `cultural`
- Canonical family: `FAM-8c52c7f87fb5` / `cultural`
- Family relation: `SUPPORTING_ARTIFACT`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `SUPPORTING`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COMPARATIVE/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/COAST.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/DEMOGRAPHIC_MOUNTAIN_REVISION.md
- Subject: `demographic_mountain`
- Canonical family: `FAM-c87a01d2510e` / `demographic_mountain`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/SPECIALIST_LINEAGES.md
- Subject: `specialist_lineages`
- Canonical family: `FAM-f3b653b9615c` / `specialist_lineages`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'MOUNTAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_PARTNERSHIP.md
- Subject: `partnership`
- Canonical family: `FAM-5af9166c8ae0` / `partnership`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/COAST.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/RIVER/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'RIVER', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/RIVER/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/DESERT/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'DESERT', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/DESERT/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_PARTNERSHIP.md
- Subject: `partnership`
- Canonical family: `FAM-5af9166c8ae0` / `partnership`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/PLAINS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/COAST/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/RIVER/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'RIVER', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/RIVER/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_PARTNERSHIP.md
- Subject: `partnership`
- Canonical family: `FAM-5af9166c8ae0` / `partnership`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/DESERT.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'DESERT', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/DESERT/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/CULTURAL_AUDIT_CHECKLIST.md
- Subject: `cultural`
- Canonical family: `FAM-8c52c7f87fb5` / `cultural`
- Family relation: `SUPPORTING_ARTIFACT`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `SUPPORTING`
- Relationship: `REVIEW`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COMPARATIVE/`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/GOVERNANCE_AND_AUTHORITY.md
- Subject: `governance_authority`
- Canonical family: `FAM-9fb17d14be42` / `governance_authority`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `REVIEW`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/COAST.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/DESERT.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'DESERT', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/DESERT/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/PLAINS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/SPECIALIST_HOUSES_CANON.md
- Subject: `specialist_houses`
- Canonical family: `FAM-5aa4e0c757fc` / `specialist_houses`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'MOUNTAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `REVIEW`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/SPECIALIST_LINEAGES.md
- Subject: `specialist_lineages`
- Canonical family: `FAM-f3b653b9615c` / `specialist_lineages`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'MOUNTAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `REVIEW`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `SUPPORTING`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COMPARATIVE/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/COMPARATIVE/FAMILY_BIRTH_CHILDHOOD_COMPARATIVE.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `SUPPORTING_ARTIFACT`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `SUPPORTING`
- Relationship: `SUPPORTING`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COMPARATIVE/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/PLAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/RIVER/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'RIVER', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/RIVER/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/PLAINS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/DESERT.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'DESERT', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/DESERT/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/COAST.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/PLAINS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'MOUNTAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/WETLANDS.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'WETLANDS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/WETLANDS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/DESERT.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'DESERT', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/DESERT/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/CULTURAL_AUDIT_CHECKLIST.md
- Subject: `cultural`
- Canonical family: `FAM-8c52c7f87fb5` / `cultural`
- Family relation: `SUPPORTING_ARTIFACT`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `SUPPORTING`
- Relationship: `REVIEW`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COMPARATIVE/`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/COAST.md
- Subject: ``
- Canonical family: `UNRESOLVED` / `UNRESOLVED`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'COAST', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `REVIEW`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/COAST/`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/PLAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'PLAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/PLAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'MOUNTAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping

## 03_PEOPLES/CULTURES/HEARTH/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `FAMILY_ROOT_OR_UNSCOPED`
- Category: `CULTURE`
- Scope: `{'region': None, 'continent': None, 'regional_scope': False}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `REVIEW`
- Action: `REVIEW`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, no safe canonical destination inferred

## 03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/FAMILY_BIRTH_CHILDHOOD.md
- Subject: `birth_childhood`
- Canonical family: `FAM-66bb4efea46c` / `family_birth_childhood`
- Family relation: `REGIONAL_SPECIALIZATION`
- Category: `CULTURE`
- Scope: `{'region': 'MOUNTAINS', 'continent': None, 'regional_scope': True}`
- Role: `AUTHORITATIVE`
- Relationship: `RELATED`
- Recommended destination: `03_PEOPLES/CULTURES/HEARTH/MOUNTAINS/`
- Action: `NO_MOVE_AUTOMATICALLY`
- Evidence: canonical identity, canonical family lineage, domain/category, scope, document role, lifecycle, domain-aware destination mapping
