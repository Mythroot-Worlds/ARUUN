...
FINAL_LABELS = {'DUPLICATE','SUPPORTING','HISTORICAL','MISPLACED','CONFLICT','RELATED','COINCIDENTAL','REVIEW','UNRESOLVED'}
LEGACY_LABELS = {'VARIANT'}

def normalize_label(label):
    value = str(label or 'UNCLASSIFIED').strip().upper()
    return 'REVIEW' if value in LEGACY_LABELS else value
...
def main():
 ...
 result={'engine':'CORE Human Adjudication Queue','mode':'REVIEW_ONLY','source_discoveries':data.get('relationships_discovered',0),'queue_size':len(review),'final_label_vocabulary':sorted(FINAL_LABELS),'legacy_label_vocabulary':sorted(LEGACY_LABELS),'instructions':['Assign a human_label only after reviewing both records and their context.','Final labels: DUPLICATE, SUPPORTING, HISTORICAL, MISPLACED, CONFLICT, RELATED, COINCIDENTAL, REVIEW, or UNRESOLVED.','VARIANT is legacy-only and must never be written as a final resolution; route it to REVIEW/context arbitration.','Do not infer canon from match strength alone.','Record a reason and preserve the relationship_id.'],'queue':review}
 (out/'CORE_ADJUDICATION_QUEUE.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
 md=['# CORE Human Adjudication Queue','','**Human review required. No labels are inferred automatically.**','',f"Source discoveries: **{result['source_discoveries']}**",f"Review queue: **{result['queue_size']}**",'','## Resolution vocabulary','- Final: `DUPLICATE`, `SUPPORTING`, `HISTORICAL`, `MISPLACED`, `CONFLICT`, `RELATED`, `COINCIDENTAL`, `REVIEW`, `UNRESOLVED`','- Legacy only: `VARIANT` — never a final resolution; route through semantic arbitration.','','| Review | Strength | Relationship | Status |','|---|---:|---|---|']
