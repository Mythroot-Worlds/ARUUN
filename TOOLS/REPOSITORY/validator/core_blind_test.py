...
Derives domain/role/signal from the actual relationship records instead of
forcing every holdout into one PEOPLES/CANON/MERGE bucket. Read-only.
"""
...
def signal(left,right,strength):
 lr=left.rsplit('/',1)[-1].lower(); rr=right.rsplit('/',1)[-1].lower()
 roles={role(left),role(right)}
 if 'HISTORICAL' in roles:return 'KEEP'
 if lr==rr:return 'DUPLICATE'
 if 'COMPARATIVE' in roles or 'TOOL' in roles or 'WORKING' in roles or 'WORKING_CANON' in roles:return 'MERGE'
 return 'MERGE'
...
def main():
 ...
 results=[]
 for r in holdout:
  left,right=r.get('left',''),r.get('right','');proposed=signal(left,right,r.get('match_strength',0));d=domain(left);d2=domain(right);domain_key=d if d==d2 else 'UNKNOWN';roles={role(left),role(right)}
  # A relationship inherits the most informative role present; mixed records are explicit.
  ...
  for h in heur:
   ...
   score=(support**0.5)*compatibility*strength_fit;scores[label]=scores.get(label,0)+score;evidence.setdefault(label,[]).append({'support':support,'proposed':hp,'strength_fit':round(strength_fit,3),'eligible':h.get('status')=='ELIGIBLE_FOR_HUMAN_REVIEW'})
  if scores:
