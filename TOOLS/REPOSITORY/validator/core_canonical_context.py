...
CONTRADICTION_PATTERNS=(r'\bcontradicts?\b',r'\binconsistent with\b',r'\bcontradiction\b',r'\bconflicts? with\b',r'\bincompatible with\b',r'\bcontrary to\b',r'\bnot true\b',r'\bis false\b',r'\bfalse claim\b',r'\bincorrect(?:ly)? states?\b')
...
def contradiction_evidence(candidate_text, canonical_claims, subject_tokens):
 """Return only explicit claim-level contradiction evidence.

 Contrastive prose (different/unlike/instead), regional variation, and temporal
 precedence are not conflicts. A conflict requires an explicit incompatibility
 assertion tied to the same subject vocabulary and a canonical claim.
 """
 lines=[re.sub(r'\s+',' ',x).strip() for x in candidate_text.splitlines() if x.strip()]
 claims=[re.sub(r'\s+',' ',x).strip() for x in canonical_claims if x.strip()]
 evidence=[]
 for line in lines:
  low=line.lower()
  if not any(re.search(p,low) for p in CONTRADICTION_PATTERNS):
   continue
  if subject_tokens and not any(t in low for t in subject_tokens):
   continue
  for claim in claims:
   shared=text_tokens(line)&text_tokens(claim)&set(subject_tokens)
   if shared:
    evidence.append({'candidate_claim':line[:500],'canonical_claim':claim[:500],'shared_subject_terms':sorted(shared),'signals':[p for p in CONTRADICTION_PATTERNS if re.search(p,low)]})
    break
 return evidence[:6]
def build_for_pair(case,root):
 ...
 def relation(ctx,text):
  ...
  return {'known_claim_count':len(claims),'token_overlap':len(tt&ct)/max(1,len(tt|ct)),'claim_overlap':sum(1 for l in claims if l.lower()[:120] in text),'conflict_evidence':contradiction_evidence(text,claims,ctx['subject_tokens'])}
 ar,br=relation(ca,at),relation(cb,bt)
 paths_u=(a+'/'+b).upper();comparative='COMPARATIVE' in paths_u;regional_markers=('/DESERT/','/RIVER/','/COAST/','/PLAINS/','/MOUNTAINS/','/WETLANDS/')
 regional_count=sum(x in paths_u for x in regional_markers);different_scope=regional_count>=2
 canonical_hint='REVIEW';hint_reason='canonical knowns collected; no relationship promoted without evidence'
 ...
 elif ar['conflict_evidence'] or br['conflict_evidence']:
  canonical_hint='CONFLICT';hint_reason='explicit claim-level contradiction evidence against repository-known claims';
...
def main():
 ...
 payload={'engine':'CORE A.C.E. Canonical Context','schema_version':'1.2','mode':'READ_ONLY','purpose':'ground document relationship decisions in canonical repository knowns before pair classification','cases':cases,'summary':{'cases':len(cases),'canonical_grounded_cases':len(cases),'hint_counts':{h:sum(c['canonical_context']['canonical_relationship_hint']==h for c in cases) for h in ('VARIANT','RELATED','SUPPORTING','HISTORICAL','CONFLICT','MISPLACED','DUPLICATE','REVIEW')},'explicit_conflict_cases':sum(bool(c['canonical_context']['a_against_knowns']['conflict_evidence'] or c['canonical_context']['b_against_knowns']['conflict_evidence']) for c in cases)},'safety':{'automatic_canon_change':False,'automatic_rule_promotion':False}}
