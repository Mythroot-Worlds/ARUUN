#!/usr/bin/env python3
"""CORE A.C.E. Detective: bounded investigation with auditable evidence rounds."""
...
def evidence_for(a,b,root,extra_targets=(),exclude=()):
    targets=[]
    for p in list(extra_targets)+related_candidates(a,b,root,exclude=set(exclude)|set(extra_targets)):
        if p not in targets: targets.append(p)
    targets=targets[:5];snippets=[];focus=['AUTHORITY','LEADERSHIP','HOUSE','VILLAGE','SETTLEMENT','REGIONAL','CONTINENT','FAMILY','BIRTH','SUPPORT','REFERENCE','HISTORICAL']
...
def missing_targets(unknown,root,a,b,used):
    """Generate concrete targets from the unanswered dimension, excluding round-one evidence."""
    ...
    for u in unknown:
        ...
        if candidates:
            target=candidates[0];targets.append(target);reasons.append({'missing':u,'dimension':dimension,'target':target,'reason':'round-one evidence did not satisfy the question-specific validity test'})
    return list(dict.fromkeys(targets)),reasons
...
def investigate(r,root):
    ...
    t1,s1=evidence_for(a,b,root);all_targets.extend(t1);all_snippets.extend(s1);e1=[x for s in s1 for x in s['excerpts']];v1=[evidence_validity(q,e1) for q in questions];all_validity.extend(v1)
    remaining=[]
    for u in unknown:
        relevant=[v for v in v1 if ('authority' in u and 'authority' in v['question'].lower()) or ('support' in u and 'support' in v['question'].lower()) or ('temporal' in u and 'temporal' in v['question'].lower())]
    ...
    rounds.append({'round':1,'trigger':'initial investigation','questions':questions,'evidence_targets':t1,'evidence_found':s1,'evidence_validity':v1,'unanswered_after_round':remaining,'decision':'answered' if questions and not remaining else ('no_investigation_needed' if not questions else 'insufficient'),'next_round_justified':bool(remaining)})

    # Round 2: only when round 1 left an actionable unknown, and only with targets derived from that gap.
    second_targets=[];missing=[];s2=[];v2=[]
    if remaining:
        second_targets,missing=missing_targets(remaining,root,a,b,all_targets)
    if remaining and second_targets:
        t2,s2=evidence_for(a,b,root,extra_targets=second_targets,exclude=all_targets);all_targets.extend(t2);all_snippets.extend(s2);e2=[x for s in s2 for x in s['excerpts']];v2=[evidence_validity(q,e2) for q in questions];all_validity.extend(v2)
        second_resolved=[]
        for u in remaining:
        ...
        rounds.append({'round':2,'trigger':'round 1 was insufficient','trigger_evidence':missing,'questions':questions,'evidence_targets':t2,'evidence_found':s2,'evidence_validity':v2,'unanswered_after_round':remaining,'decision':'answered' if second_resolved and not remaining else 'insufficient','next_round_justified':False})
