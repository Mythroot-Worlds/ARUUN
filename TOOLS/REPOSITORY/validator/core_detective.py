...
def direct_case(r,triage_result):
 evidence=triage_result.get('evidence',{})
 return {'relationship_id':r.get('relationship_id'),'documents':{'a':r.get('left',''),'b':r.get('right','')},'role':'case_solver','triage_status':'DIRECT','triage_decision':triage_result.get('decision','REVIEW'),'triage_reason':triage_result.get('reason','resolved by document triage'),'triage_evidence':evidence,'relationship_reasoning':{'decision':triage_result.get('decision','REVIEW'),'confidence':'STRUCTURAL','decision_basis':triage_result.get('reason','resolved by document triage'),'candidates':[triage_result.get('decision','REVIEW')],'viable_relationships':[triage_result.get('decision','REVIEW')],'unresolved_dimensions':[]},'case_question':{'question':'What is this document relationship and where does it belong?','status':'resolved_by_triage'},'investigation_rounds':0,'investigation_rounds_detail':[],'evidence_targets':[],'evidence_claims':[],'unknown_after':[],'evidence_updates':[{'type':'triage_update','effect':'resolved_without_deep_investigation','basis':triage_result.get('reason','resolved by document triage')}],'evidence_quality_summary':{'direct_claims':0,'grounded_direct_claims':0,'pair_resolving_claims':0,'contextual_claims':0,'unique_claims':0},'stop_reason':'clear relationship resolved by document triage; Batman not required','safety':{'self_training':False,'automatic_rule_promotion':False,'automatic_canon_change':False}}
def investigate(r,root,robin,pool,triage_result):
 if triage_result.get('triage_status')=='DIRECT': return direct_case(r,triage_result)
 ...
 resolved2=[v['dimension'] for v in second_vals if v['answers_question']];remaining=[d for d in unresolved if d not in resolved2];rounds=[{'round':1,'trigger':'ambiguous case investigation','evidence_targets':targets,'evidence_validity':vals,'unanswered_after_round':unresolved}]
...
def main():
 ...
 payload={'engine':'CORE A.C.E. Detective','schema_version':'3.5','mode':'READ_ONLY','purpose':'simple document triage first; deep Batman/Robin investigation only for genuinely ambiguous cases','cases':cases,'summary':summary,'safety':{'human_validation_required':True,'automatic_canon_change':False,'automatic_rule_promotion':False}}
