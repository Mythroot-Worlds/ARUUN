#!/usr/bin/env python3
"""CORE A.C.E. Detective: bounded investigation with grounded evidence claims."""
...
def extract_entities(text):
 # Lightweight grounding: preserve explicit document/entity phrases instead of treating a keyword as a relation by itself.
 vals=[]
...
def claim_from(path,text,dimension,question_entities=()):
 ...
 # A keyword match alone is never direct. Direct evidence must contain a relation-like pattern and at least one grounded entity phrase.
...
def investigate(r,root,pool=DEFAULT_POOL_SIZE):
 ...
 direct=sum(c['claim_type']=='DIRECT' for c in all_claims);context=sum(c['claim_type']=='CONTEXTUAL' for c in all_claims);unique=len({(c['source'],c['passage'],c['dimension']) for c in all_claims});grounded=sum(c['grounded'] for c in all_claims)
