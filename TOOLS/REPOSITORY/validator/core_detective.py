#!/usr/bin/env python3
"""CORE A.C.E. Detective: bounded investigation with evidence claim chains."""
...
def candidates(a,b,root,exclude=(),pool=DEFAULT_POOL_SIZE):
 ...
 for p in files(root):
  ...
  for k in FOCUS:
   score += 2*len((set(ta[k])|set(tb[k])) & set(t[k]))
...
def claim_from(path,text,dimension):
 ...
 direct=bool(re.search(patterns[dimension],u,re.S));contextual=bool(any(w in u for w in FOCUS.get(dimension,[])))
...
def validity(question,dimension,claims):
 relevant=[c for c in claims if c['dimension']==dimension];direct=[c for c in relevant if c['claim_type']=='DIRECT'];context=[c for c in relevant if c['claim_type']=='CONTEXTUAL'];sources={c['source'] for c in context};answered=bool(direct) or len(sources)>=2;quality=1.0 if direct else (.75 if len(sources)>=2 else (.5 if context else 0.0))
