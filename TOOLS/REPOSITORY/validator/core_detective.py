...
def candidates(a,b,root,exclude=(),pool=DEFAULT_POOL_SIZE):
 ...
 for p in files(root):
  ...
  t=terms(read(p,root));score=sum(2*(len(set(ta[k])|set(tb[k])) & len(set(t[k]))) for k in FOCUS)
...
def validity(question,dimension,claims):
 ...
 answered=bool(direct) or len(sources)>=2;quality=1.0 if direct else (.75 if len(sources)>=2 else (.5 if context else 0.0))
...
def investigate(r,root,pool=DEFAULT_POOL_SIZE):
 ...
 questions=[{'unknown':u,'question':question_for(u)[1],'dimension':question_for(u)[0]} for u in unknown];targets=candidates(a,b,root,pool=pool);rounds=[];all_claims=[];resolved=[]
 vals=[]
 for q in questions:
