#!/usr/bin/env python3
"""CORE A.C.E. Detective: evidence-seeking investigation with evidence validity."""
...
def evidence_validity(question,excerpts):
    ...
    if 'authority' in q:
        checks=[('explicit_authority',bool(re.search(r'\b(LEAD|LEADER|GOVERN|AUTHORITY|HEAD|COUNCIL)\b',text))),('organizational_scale',bool(re.search(r'\b(VILLAGE|SETTLEMENT|REGION|REGIONAL|CONTINENT|CONTINENTAL)\b',text)))]
    ...
    return {'question':question,'checks':[{'test':k,'passed':v} for k,v in checks],'answerability':round(passed/len(checks),2) if checks else 0.0,'answers_question':bool(checks) and passed==len(checks)}
...
def investigate(r,root):
    ...
    targets,snippets=evidence_for(a,b,root);all_excerpts=[x for s in snippets for x in s['excerpts']];validity=[evidence_validity(q,all_excerpts) for q in questions]
    resolved=[]
    for u in unknown:
    ...
    updates=[]
    if resolved: updates.append({'type':'evidence_update','effect':'reduced_uncertainty','resolved_unknowns':resolved,'basis':'retrieved evidence passed the question-specific answerability test'})
    ...
    return {'relationship_id':r.get('relationship_id'),'documents':{'a':a,'b':b},'known':known,'unknown_before':sorted(set(unknown)),'assumptions':sorted(set(assumptions)),'questions':questions,'evidence_targets':targets,'evidence_found':snippets,'evidence_validity':validity,'evidence_updates':updates,'unknown_after':sorted(set(remaining)),'hypotheses':hypotheses,'challenge_questions':[f'What evidence would falsify {h["label"]}?' for h in hypotheses],'investigation_rounds':2,'stop_reason':'targeted evidence pass completed; human review remains authoritative','safety':{'self_training':False,'automatic_rule_promotion':False,'automatic_canon_change':False}}
