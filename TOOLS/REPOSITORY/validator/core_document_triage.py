...
def placement(ident):
    """Return identity plus the repository parent implied by document layer.

    Scope answers *what area the document describes*; identity_layer answers
    *which repository layer the document itself occupies*. A regional index
    document such as HEARTH/PLAINS.md therefore stays at HEARTH/, while a
    regional specialization such as HEARTH/PLAINS/FAMILY_BIRTH_CHILDHOOD.md
    belongs under HEARTH/PLAINS/.
    """
    path=Path(ident['path'])
    scope=ident.get('scope',{})
    region=scope.get('region')
    layer=ident.get('identity_layer')
    parent=path.parent.as_posix()
    if layer=='CANONICAL_ROOT' and ident.get('content_type')=='CULTURE' and region:
        # A region-named canonical root file is the index for that region, not
        # a specialization contained by the region directory.
        if path.stem.upper()==region:
            parent=f'03_PEOPLES/CULTURES/HEARTH'
        elif 'COMPARATIVE' in path.parts:
            parent='03_PEOPLES/CULTURES/HEARTH/COMPARATIVE'
    elif layer=='REGIONAL_SPECIALIZATION' and region:
        parent=f'03_PEOPLES/CULTURES/HEARTH/{region}'
    elif layer=='HISTORICAL_ARTIFACT' and '07_ARCHIVE/HISTORICAL' in parent.upper():
        parent='07_ARCHIVE/HISTORICAL'
    elif layer=='SUPPORTING_ARTIFACT' and 'COMPARATIVE' in path.as_posix().upper():
        parent='03_PEOPLES/CULTURES/HEARTH/COMPARATIVE'
...
def main():
    ...
    payload={'engine':'CORE A.C.E. Document Triage','schema_version':'3.6','mode':'READ_ONLY','purpose':'identify and place documents before describing pair relationships','cases':results,'calibration':cal,'summary':summary,'safety':{'human_validation_required':True,'automatic_canon_change':False,'automatic_rule_promotion':False}}
