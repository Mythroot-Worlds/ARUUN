    ...
    *which repository layer the document itself occupies*. A regional index
    ...
    if layer=='CANONICAL_ROOT' and ident.get('content_type')=='CULTURE' and region:
        if path.stem.upper()==region:
            parent=f'{HEARTH_ROOT}'
        elif 'COMPARATIVE' in path.parts:
            parent=f'{HEARTH_ROOT}/COMPARATIVE'
    elif layer=='REGIONAL_SPECIALIZATION' and region:
        parent=f'{HEARTH_ROOT}/{region}'
...
def _is_hearth_culture(ident):
    return ident.get('content_type')=='CULTURE' and ident.get('path','').upper().startswith(HEARTH_ROOT)

def _is_regional_root(ident):
    path=Path(ident.get('path',''))
    region=ident.get('scope',{}).get('region')
...
def same_category(result):
    return result.get('layers',{}).get('category',{}).get('state')=='SAME'
