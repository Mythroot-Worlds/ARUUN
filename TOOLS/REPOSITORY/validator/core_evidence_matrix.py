#!/usr/bin/env python3
"""CORE A.C.E. evidence matrix: deciding-factor reasoning over relationships."""
...
def matrix(a,b):
 ...
 factors=factor_snapshot(a,b)
 return {'scope_a':sa,'scope_b':sb,'scope_relation':rel,'function_a':sorted(fa),'function_b':sorted(fb),'function_overlap':sorted(fa&fb),'function_difference':sorted(fa^fb),'deciding_factors':factors['dimensions'],'principle':factors['principle']}
...
def classify(m,prior):
 ...
 if m['scope_relation']=='SAME' and m['function_a']==m['function_b'] and m['function_a']: return 'DUPLICATE'
...
def main():
 ...
 for r in blind.get('predictions',[]):
  a=read(r.get('left'),root)+' '+r.get('left','').upper();b=read(r.get('right'),root)+' '+r.get('right','').upper();m=matrix(a,b);pred=classify(m,r.get('predicted_classification','RELATED'));res.append({'relationship_id':r.get('relationship_id'),'matrix':m,'matrix_prediction':pred,'decision_factors':relationship_test(pred,m['deciding_factors']),'prior_prediction':r.get('predicted_classification'),'changed':pred!=r.get('predicted_classification')})
 report={'engine':'CORE A.C.E. Evidence Matrix','schema_version':'2.0','mode':'READ_ONLY','predictions':res,'summary':{'holdouts':len(res),'changed':sum(x['changed'] for x in res),'distinct_predictions':len(set(x['matrix_prediction'] for x in res)),'factor_dimensions':15,'safety':{'automatic_rule_promotion':False,'automatic_canon_change':False,'human_validation_required':True}}}
