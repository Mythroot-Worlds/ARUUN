...
def base_document_key(path):
    """Normalize revision/version suffixes without erasing subject identity."""
    stem=Path(path).stem.lower()
    stem=re.sub(r"(?:_v\d+(?:\.\d+)?)$|(?:_revision_?\d+)$|(?:_rev_?\d+)$|(?:_draft_?\d+)$", "", stem)
    stem=re.sub(r"_comparative$", "", stem)
    return (str(Path(path).parent).lower(), stem)

def layer_profile(path,text):
    i=identify(path)
    return {'category':i['content_type'],'subject':i['subject'],'context':i['scope'],'purpose':i['role'],'content_fingerprint_size':len(words(text)),'path':path,'base_document_key':base_document_key(path)}
...
def compare(a_path,a_text,b_path,b_text):
    ...
    same_revision_lineage=a['base_document_key']==b['base_document_key'] and same_context
    # A revision/expansion can contain substantially more text than its predecessor.
    # Use containment as evidence of retained information, but do not require an
    # arbitrary Jaccard floor when the path itself establishes explicit revision lineage.
    ...
    layers={
      ...
      'revision_lineage':{'state':'SAME' if same_revision_lineage else 'DIFFERENT','left':a['base_document_key'],'right':b['base_document_key']}
    }
    if same_category and same_subject and same_context and informational_equivalence:
        decision='DUPLICATE' if stats['jaccard']>=0.85 and max(stats['containment_left'],stats['containment_right'])>=0.90 and not same_revision_lineage else 'VARIANT'
