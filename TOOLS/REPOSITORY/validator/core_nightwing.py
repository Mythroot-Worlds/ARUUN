...
def synth(case,oracle):
 ...
 for d in dims:
  if d.get('state')=='AGREE': verdict='CONVERGENT'
  elif d.get('state') in {'BATMAN_ONLY','ROBIN_ONLY'}: verdict='DIVERGENT'
  else: verdict='UNRESOLVED'
  out.append({'dimension':d.get('dimension'),'verdict':verdict,'basis':{'batman_answered':d.get('batman_answered',False),'robin_semantic_support':d.get('robin_semantic_support',False),'oracle_context':bool(oracle.get('oracle',{}).get('association_candidates'))}})
