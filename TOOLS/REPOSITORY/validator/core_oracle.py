...
def case_oracle(case,root):
 ...
 # Lexical overlap is retained only as a retrieval clue. It is never promoted to a semantic relation.
 for t in shared[:12]:
  o=observation('oracle',t,'retrieval_clue',t,a,'shared lexical term; retrieval clue only',.15,association_type='lexical_clue',semantic_status='UNVERIFIED');add(board,o);obs.append(o)
 ...
 factor=factor_snapshot(ta,tb)
 board['oracle']={'shared_terms':shared,'retrieval_clues':len(obs),'association_candidates':[{'path':p,'overlap':s,'source_role':source_role(p),'canonical_status':canonical_status(read(p,root))} for s,p in candidates],'deciding_factor_snapshot':factor['dimensions'],'strategy':'broad retrieval clue -> factor-aware narrowing -> provenance/canon inspection','semantic_associations_created':0}
 return board

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--root',default='.');ap.add_argument('--out',default='TOOLS/REPOSITORY/REPORTS');x=ap.parse_args();root=Path(x.root).resolve();out=root/x.out;det=load(out/'CORE_DETECTIVE_REPORT.json',{'cases':[]});boards=[case_oracle(c,root) for c in det['cases']];payload={'engine':'CORE A.C.E. Oracle','schema_version':'2.0','mode':'READ_ONLY','purpose':'canonical/context retrieval, provenance, and information relay; Oracle does not decide semantic relationships','cases':boards,'summary':{'cases':len(boards),'retrieval_clues':sum(len(b['items']) for b in boards),'cases_with_repository_candidates':sum(bool(b['oracle']['association_candidates']) for b in boards),'semantic_associations_created':0},'safety':{'human_validation_required':True,'automatic_canon_change':False,'automatic_rule_promotion':False,'lexical_overlap_is_not_semantic_evidence':True}}
 out.mkdir(parents=True,exist_ok=True);(out/'CORE_ORACLE_REPORT.json').write_text(json.dumps(payload,indent=2),encoding='utf-8');(out/'CORE_ORACLE_REPORT.md').write_text('# CORE Oracle Report\n\nRead-only information and provenance service. Lexical overlap is retrieval evidence only.\n\n'+json.dumps(payload['summary'],indent=2),encoding='utf-8');print(json.dumps(payload['summary'],indent=2))
