...
def layered_adjudication(layered):
 ...
 layers=layered.get('layers',{})
 basis=[]
 if s['category']!='SAME': return {'decision':'REVIEW','confidence':'LOW','basis':'Category differs or is unresolved; deep investigation required.','decisive_layers':['category'],'rejected_alternatives':[]}
 if s['subject']=='DIFFERENT': return {'decision':'RELATED','confidence':'HIGH','basis':'Same broad category but different subject; the artifacts are not variants of the same information.','decisive_layers':['category','subject'],'rejected_alternatives':['VARIANT','DUPLICATE']}
 # Comparative/supporting material is a role distinction, not a semantic duplicate.
 # A comparative document is explicitly a derived secondary layer and should not
 # be forced through the VARIANT/RELATED path merely because it shares a subject.
 left_role=str(layers.get('purpose',{}).get('left','')).upper(); right_role=str(layers.get('purpose',{}).get('right','')).upper()
 left_path=str(layers.get('path',{}).get('left','')).upper(); right_path=str(layers.get('path',{}).get('right','')).upper()
 ...
 if comparative and left_role!=right_role and 'SUPPORTING' in {left_role,right_role}:
  return {'decision':'SUPPORTING','confidence':'HIGH','basis':'One document is an explicitly comparative/derived secondary layer while the other is authoritative source material for the same subject.','decisive_layers':['subject','purpose','content'],'rejected_alternatives':['VARIANT','DUPLICATE','RELATED']}
 if s['scope']=='DIFFERENT':
  if s['content'] in {'NEAR_SAME','SAME'} and s['information']=='HIGH': return {'decision':'RELATED','confidence':'HIGH','basis':'Same subject but different contextual scope; regional/contextual identity prevents VARIANT.','decisive_layers':['subject','context'],'rejected_alternatives':['VARIANT','DUPLICATE']}
 ...
 if s['subject']=='SAME' and s['scope']=='SAME':
  if s['revision']=='SAME_REVISION_LINEAGE': return {'decision':'VARIANT','confidence':'HIGH','basis':'Same subject and context with explicit revision lineage; version/detail changes are variants.','decisive_layers':['subject','context','revision'],'rejected_alternatives':['RELATED']}
 ...
 if s['revision']=='SAME_REVISION_LINEAGE': return {'decision':'HISTORICAL','confidence':'HIGH','basis':'Explicit revision lineage explains the relationship but the current layered identity is not otherwise equivalent.','decisive_layers':['revision'],'rejected_alternatives':['RELATED']}
 return None
def evaluate_relationships(robin,layered=None):
 la=layered_adjudication(layered)
 if la and la['decision'] in {'VARIANT','RELATED','HISTORICAL','SUPPORTING'}:
