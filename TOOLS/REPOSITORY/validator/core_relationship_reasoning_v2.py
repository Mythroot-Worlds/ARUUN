...
def layered_constraints(layered):
 ...
 if s['scope']=='DIFFERENT': c['excluded']|={'VARIANT','DUPLICATE','CONFLICT'};c['supported']|={'RELATED','MISPLACED','SUPPORTING'};c['decisive_layers'].append('context');c['basis'].append('different context vetoes VARIANT/DUPLICATE/CONFLICT but does not itself force RELATED')
 elif s['scope']=='UNKNOWN': c['excluded']|={'VARIANT','DUPLICATE','CONFLICT'};c['decisive_layers'].append('context');c['basis'].append('context unresolved')
...
def evaluate_relationships(robin,layered=None):
 c=layered_constraints(layered) or {'excluded':set(),'supported':set(),'decisive_layers':[],'basis':[]}
 cs=[candidate(n,m,robin) for n,m in MODELS.items()]
 for x in cs:
  n=x['relationship']
  if n in c['excluded']:
   x['status']='DISQUALIFIED';x['blocking_factors'].append('layered ontology exclusion: '+'; '.join(c['basis']))
  elif n in c['supported']:
   x['score']+=8;x['supporting_factors'].append('layered ontology support')
 cs.sort(key=lambda x:(x['status']=='VIABLE',x['score']),reverse=True)
 viable=[x for x in cs if x['status']!='DISQUALIFIED']
 best=viable[0] if viable else None
 competitors=viable[1:3] if best else []
 decisive_unknown=sorted(set(best['decisive_unknowns']) & set(c['decisive_layers'])) if best else c['decisive_layers']
 if best and not decisive_unknown and best['status']=='VIABLE' and (not competitors or best['score']>competitors[0]['score']+3):
  decision=best['relationship'];confidence='HIGH' if best['score']>=10 else 'MEDIUM';basis='Layered constraints narrow the field; Robin evidence discriminates the surviving hypotheses.'
 else:
  decision='REVIEW';confidence='LOW' if decisive_unknown else 'MEDIUM';basis='Layered constraints leave multiple plausible relationships or a decisive factor unresolved.'
 strongest_alt=competitors[0] if competitors else None
 alt_reason='no surviving alternative'
 if strongest_alt:
  if strongest_alt['status']=='DISQUALIFIED': alt_reason='rejected by layered ontology constraints'
  elif strongest_alt['blocking_factors']: alt_reason='blocked by: '+'; '.join(str(x) for x in strongest_alt['blocking_factors'])
  elif strongest_alt['decisive_unknowns']: alt_reason='requires unresolved factors: '+', '.join(str(x) for x in strongest_alt['decisive_unknowns'])
  elif best: alt_reason=f"score {strongest_alt['score']} did not clear winning margin over {best['score']}"
 return {'decision':decision,'confidence':confidence,'decision_basis':basis,'reasoning_mode':'layered_constraints_then_factor_adjudication','candidates':cs,'viable_relationships':[x['relationship'] for x in viable],'uncertain_relationships':[x['relationship'] for x in cs if x['status']=='UNCERTAIN'],'decisive_unknowns':decisive_unknown,'non_decisive_unknowns':sorted(d for d in robin if state(robin,d)=='UNKNOWN' and d not in decisive_unknown),'unresolved_dimensions':sorted(d for d in robin if state(robin,d)=='UNKNOWN'),'rejected_alternatives':[x['relationship'] for x in cs if x['status']=='DISQUALIFIED'],'decisive_layers':c['decisive_layers'],'strongest_alternative':strongest_alt['relationship'] if strongest_alt else None,'strongest_alternative_status':strongest_alt['status'] if strongest_alt else None,'strongest_alternative_reason':alt_reason,'legacy_exemplar_override':False,'layered_constraints':{'excluded':sorted(c['excluded']),'supported':sorted(c['supported']),'basis':c['basis']}}
