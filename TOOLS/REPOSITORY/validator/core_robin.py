...
Robin is intentionally complementary to Batman. It does not decide canon or
replace repository evidence retrieval. It extracts lightweight local
subject/relation/object patterns, surrounding context, and ambiguity signals
that Batman can use for cross-checking.
"""
...
MAX_WINDOW=3
RELATIONS={
    'authority': ['govern','rule','lead','oversee','command','authority','governance','council','head'],
    'support': ['support','inform','reference','derive','based','build','expand','adapt','source'],
    'temporal': ['revise','replace','supersede','previous','former','earlier','current','older','version'],
    'scope': ['continent','regional','region','settlement','village','local','hearth','mountain','river','plains'],
}
...
def named_spans(sentence):
    # Lightweight noun/entity candidates. Deliberately conservative; this is a
    # cross-check signal, not a claim that capitalization equals an entity.
    vals=[]
...
def local_analysis(path,root,dimension):
    ...
    for i,s in enumerate(ss):
        ...
        syntax_signal=bool(re.search(r'\b(?:is|are|has|have|governs?|rules?|leads?|supports?|informs?|derives?|replaces?|supersedes?|belongs?|contains?|within|under|from|for)\b',s,re.I))
...
def robin_case(case,root):
    docs=[case.get('documents',{}).get('a',''),case.get('documents',{}).get('b','')]
    dims=[q.get('dimension','general') for q in case.get('questions',[])] or ['authority','support','temporal']
    analyses=[]
    for d in dict.fromkeys(dims):
        for p in docs: analyses.extend(local_analysis(p,root,d))
    # Cross-check only: Robin reports whether syntax/context gives a relation
    # signal and whether the same dimension has ambiguity or contradiction cues.
    ...
    for d,items in by.items():
        ...
        results[d]={'relation_observations':len(items),'syntax_supported':len(explicit),'entity_observations':entities,'ambiguity_signals':ambiguity,'supports_semantic_relation':bool(explicit and entities>0),'confidence':'high' if explicit and entities>0 and ambiguity==0 else ('medium' if explicit else 'low')}
    return {'relationship_id':case.get('relationship_id'),'documents':case.get('documents',{}),'robin_results':results,'observations':analyses,'method':'local syntax + contextual window + conservative entity/relation candidates','independence':'Robin does not consume Batman conclusions; it analyzes source text independently'}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--root',default='.');ap.add_argument('--out',default='TOOLS/REPOSITORY/REPORTS');x=ap.parse_args();root=Path(x.root).resolve();out=root/x.out;report=load(out/'CORE_DETECTIVE_REPORT.json',{'cases':[]});cases=[robin_case(c,root) for c in report.get('cases',[])];summary={'cases':len(cases),'semantic_relation_cases':sum(any(v['supports_semantic_relation'] for v in c['robin_results'].values()) for c in cases),'high_confidence_dimensions':sum(sum(v['confidence']=='high' for v in c['robin_results'].values()) for c in cases),'ambiguity_signals':sum(sum(v['ambiguity_signals'] for v in c['robin_results'].values()) for c in cases)};payload={'engine':'CORE A.C.E. Robin','schema_version':'1.0','mode':'READ_ONLY','purpose':'independent syntax/context semantic cross-check against Detective evidence','cases':cases,'summary':summary};(out/'CORE_ROBIN_REPORT.json').write_text(json.dumps(payload,indent=2),encoding='utf-8');print(json.dumps(summary,indent=2))
