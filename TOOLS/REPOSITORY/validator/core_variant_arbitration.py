...
def _same_or_unspecified(a,b):
    """A missing qualifier is neutral; an explicit disagreement is not."""
    na,nb=_norm(a),_norm(b)
...
def _claims_from_units(units):
    return [u.get("normalized") or u.get("text") or u for u in units if isinstance(u,dict)]
