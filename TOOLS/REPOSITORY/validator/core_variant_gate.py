...
def discover_same_scope_pairs(root):
    """Find possible variants that broad relationship discovery may omit."""
    files=[]
    base=root/'03_PEOPLES/CULTURES/HEARTH'
...
def candidate_key(identity):
    # Candidate discovery is deliberately broader than VARIANT arbitration.
    # Same structural scope is required; subject/role are used only to limit
    # the search. The final identity gate below remains authoritative.
    subject_tokens=set(re.findall(r'[a-z0-9]+',identity.get('subject') or ''))
    role_tokens=set(re.findall(r'[a-z0-9]+',identity.get('role') or ''))
...
def main():
    ...
    def assess_pair(left_rel,right_rel,source='relationship_discovery'):
        ...
        same_identity,identity_reasons=identity_match(left,right)
        comparison=compare_units(left['information_units'],right['information_units'])
        assessment=assess_variant(left,right)
        # Structural identity is a hard gate. Semantic equivalence can never
        # override a scope/role/subject mismatch.
        if not same_identity:
            assessment.eligible=False
        x={'relationship_id':f'VAR-{abs(hash(key)) & 0xffffffff:08x}','left':left_rel,'right':right_rel,'source':source,
           'left_identity':{k:left[k] for k in ('entity','population','region','subregion','subject','role','purpose','scope')},
           'right_identity':{k:right[k] for k in ('entity','population','region','subregion','subject','role','purpose','scope')},
           'identity_match':same_identity,'identity_reasons':identity_reasons,
    ...
    for scope,items in by_scope.items():
        for i,left in enumerate(items):
            for right in items[i+1:]:
                a=candidate_key(ctx[left]);b=candidate_key(ctx[right])
                # Same-scope candidates with lexical identity evidence enter the
                # semantic stage. Cross-scope pairs never enter this path.
                if a & b:
                    assess_pair(left,right,'same_scope_identity_candidate')

    result={'engine':'CORE VARIANT Gate','schema_version':'4.0','mode':'READ_ONLY',
      'definition':'VARIANT means near-equivalent informational content for the same structural identity, scope, document role, purpose, and applicable time. Wording, presentation, formatting, and modest detail may differ. Similar information across different regions or roles is not VARIANT.',
      'candidate_count':len(rows),'variant_candidates':eligible,'rejected_variant_candidates':rejected,'relationships':rows,
      'identity_gate':{'authoritative':True,'fields':['entity','population','region','subregion','subject','role','purpose','scope'],'semantic_similarity_cannot_override':True},
    ...
    md=['# CORE VARIANT Gate','','VARIANT is informational near-equivalence within the same structural identity. Similar subject matter across different regions or document roles is **not** VARIANT.','',f"Candidates assessed: **{len(rows)}**",f"Eligible VARIANT candidates: **{eligible}**",f"Rejected VARIANT candidates: **{rejected}**",'','## Identity gate','- Entity','- Population','- Regional scope','- Subregion','- Subject','- Document role','- Purpose','- Applicable time when explicitly specified','','Identity is authoritative: semantic similarity cannot override a structural identity mismatch.','', 'Same-scope candidate discovery remains broader than VARIANT arbitration so true variants are not lost merely because generic relationship discovery favors other relationships.']
