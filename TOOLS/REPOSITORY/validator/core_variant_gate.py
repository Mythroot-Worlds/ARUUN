#!/usr/bin/env python3
"""Apply evidence-gated VARIANT arbitration using first-class document identity."""
...
def doc_context(root,rel,claims,units):
    legacy=resolve_identity(root,rel);identity=identify(rel)
    return {**legacy,'subject':identity['subject'] or legacy.get('subject'),'content_type':identity['content_type'],'structural_scope':identity['scope'],'claims':for_source(rel,claims,'claims'),'information_units':for_source(rel,units,'units')}
