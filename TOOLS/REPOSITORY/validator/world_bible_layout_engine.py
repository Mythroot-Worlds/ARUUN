...
# Mythroot's current repository structure, expressed as role/schema rules rather
# than pairwise similarity rules. These are intentionally conservative.
ROLE_RULES = [
...
def world_bible_categories(body):
...
def version(path, body):
    versions=[int(x) for x in re.findall(r"\bv(\d+(?:\.\d+)?)\b", body, re.I)]
    versions += [int(x) for x in re.findall(r"[_ -]v(\d+)\b", path, re.I)]
...
def canonical_status(body):
    ...
    for marker in CANON_MARKERS:
        if marker in u: hits.append(marker)
    if "STATUS:" in u:
...
def assess(path, root, bible_categories):
    ...
    present=[t for t in terms if t in u]
    missing=[] if not terms else [t for t in terms if t not in u]
    ...
    return {
        "path":path,"file_type":ftype,"role":role,"expected_category":expected_label,
        "proper_location":proper_location,"version":ver,"canonical_status":status,
        "world_bible_category_candidates":bible_categories[:50],
        "schema_signal":{"matched":present,"missing":missing,"coverage":round(structure_score,3)},
        "conflicts":flags,
        "comparison_mode":"DOCUMENT_TO_WORLD_BIBLE_SCHEMA",
        "cross_file_comparison_used":False,
        "provenance":{"source":"repository_file","world_bible":"00_MASTER/WORLD_BIBLE.md"}
    }
...
def main():
    ...
    counts=Counter(r["file_type"] for r in rows); conflicts=Counter(c for r in rows for c in r["conflicts"])
    ...
    for r in rows:
        if r['conflicts']:
            md.append(f"- **{r['path']}** — {', '.join(r['conflicts'])}")
